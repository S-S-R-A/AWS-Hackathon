...
def generate_presigned_url_for_mp3():
    try:
        mp3_key = 'output/mp3/translated_output.mp3'
        # Check if the MP3 file exists
        s3_client.head_object(Bucket=S3_BUCKET, Key=mp3_key)
        
        # If exists, generate pre-signed URL
        mp3_url = s3_client.generate_presigned_url(
            'get_object',
        ...
        app.logger.error(f"Error generating pre-signed URL for MP3: {e}")
        return None
...
def upload_and_process():
    ...
    try:
        ...
        # Generate pre-signed URL for MP3
        mp3_url = generate_presigned_url_for_mp3()
        ...
        return jsonify({
            ...
            "predicted_label": prediction if prediction else "Unknown",
            "confidence": confidence if confidence else 0,
            "mp3_url": mp3_url  # Include the MP3 URL if available
        }), 200
...
# New Endpoint to Get MP3 URL
@app.route('/get-mp3-url', methods=['GET'])
def get_mp3_url():
    mp3_url = generate_presigned_url_for_mp3()
    if mp3_url:
